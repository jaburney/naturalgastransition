# ------------------------------------------------------------------------------------------
# Analysis - County with Neighbors.R
# Produces basic county-level analyses in the paper
# ------------------------------------------------------------------------------------------
...
# Intensity
# ------------------------------------------------------------------------------------------
      # final.county.data$load_coaloff = final.county.data$load_coaloff25+final.county.data$load_coaloff50+final.county.data$load_coaloff100+final.county.data$load_coaloff200
      # final.county.data$load_ngon = final.county.data$load_ngon25+final.county.data$load_ngon50+final.county.data$load_ngon100+final.county.data$load_ngon200
      # final.county.data$load_coalon = final.county.data$load_coalon25+final.county.data$load_coalon50+final.county.data$load_coalon100+final.county.data$load_coalon200
      # 
      # rhs_int = str_replace_all(str_replace_all(rhs,"n_","load_"),"tot_","load_")
      # 
      # # Loop over models and outcomes
      # for (j in 1:length(rhs_int)) {
      #   tmp = list()
      #   tmp.ivpm = list()
      #   tmp.ivpmsurf = list()
      #   tmp.ivpmo3surf = list()
      ...
      #   }
      #   names(tmp) = names(tmp.ivpm) = names(tmp.ivpmsurf) = names(tmp.ivpmo3surf) = varlist
      #   assign(paste("m.int.",j,sep=""),tmp)
      #   assign(paste("m.int.ivpm.",j,sep=""),tmp.ivpm)
      #   assign(paste("m.int.ivpmsurf.",j,sep=""),tmp.ivpmsurf)
      #   assign(paste("m.int.ivpmo3surf.",j,sep=""),tmp.ivpmo3surf)
      # }
      # 
      # # put in slightly nicer format for tables
      # ind = c(1:length(varlist))
      # GWH <- function(x) (x * 1e9)
      # 
      # for (i in 1:length(rhs_int)) {
      #   vv = get(paste("m.int.",i,sep=""))
      #   vv.ivpm = get(paste("m.int.ivpm.",i,sep=""))
      #   vv.ivpmsurf = get(paste("m.int.ivpmsurf.",i,sep=""))
      #   vv.ivpmo3surf = get(paste("m.int.ivpmo3surf.",i,sep=""))
      #   stargazer(vv[ind],out=paste("Tables/m",i,".int.tex",sep=""),style="qje",apply.coef=GWH,apply.se=GWH)
      #   stargazer(vv.ivpm[ind],out=paste("Tables/m_ivpm_",i,".int.tex",sep=""),style="qje")
      #   stargazer(vv.ivpmsurf[ind],out=paste("Tables/m_ivpmsurf_",i,".int.tex",sep=""),style="qje")
      #   stargazer(vv.ivpmo3surf[ind],out=paste("Tables/m_ivpmo3surf_",i,".int.tex",sep=""),style="qje")
      # }
      # 
      # # Get IVs nice format for plotting -- First PM
      # for (j in 1:length(rhs_int)) {
      #   nm1 = paste("ivpmmod.int.",j,sep="")
      #   nm2 = paste("ivpmsurfmod.int.",j,sep="")
      #   tmp1 = tmp2 = matrix(NA,ncol=4,nrow=length(varlist))
      #   df1 = get(paste("m.int.ivpm.",j,sep=""))
      #   df2 = get(paste("m.int.ivpmsurf.",j,sep=""))
      #   for (i in 1:length(varlist)) {
      #     tmp1[i,] = summary(df1[[i]])$coefficients
      #     tmp2[i,] = summary(df2[[i]])$coefficients
      #   }
      ...
      # # Get IVs nice format for plotting -- PM and O3surf
      # for (j in 1:length(rhs_int)) {
      #   nm1 = paste("ivpmo3surfmod.int.",j,".pm",sep="")
      #   nm2 = paste("ivpmo3surfmod.int.",j,".o3surf",sep="")
      #   tmp1 = tmp2 = matrix(NA,ncol=4,nrow=length(varlist))
      #   df = get(paste("m.int.ivpmo3surf.",j,sep=""))
      #   for (i in 1:length(varlist)) {
      #     tmp1[i,] = summary(df[[i]])$coefficients[1,]
      #     tmp2[i,] = summary(df[[i]])$coefficients[2,]
      #   }
      ...
      # # Get Reduced Form models in nice format for plotting -- single variable
      # for (j in 1:length(rhs_int)) {
      #   nm = paste("mod.int.",j,sep="")
      #   tmp = matrix(NA,ncol=4,nrow=length(varlist))
      #   df = get(paste("m.int.",j,sep=""))
      #   for (i in 1:length(varlist)) {
      #     tmp[i,] = summary(df[[i]])$coefficients[1,]
      #   }
      #   row.names(tmp) = varlist
      ...
      # # Get nice format for plotting - multiple vars
      # for (j in c(9,10,19,22)) {
      #   nm1 = paste("mod.int.",j,".coal",sep="")
      #   nm2 = paste("mod.int.",j,".ng",sep="")
      #   nm3 = paste("mod.int.",j,".newcoal",sep="")
      #   tmp1 = tmp2 = tmp3 = matrix(NA,ncol=4,nrow=length(varlist))
      #   df = get(paste("m.int.",j,sep=""))
      #   for (i in 1:length(varlist)) {
      #     tmp1[i,] = summary(df[[i]])$coefficients[1,]
