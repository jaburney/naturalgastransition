...
# -------------------------------------------------------------------------------------------------------------------
# Compares first stage / reduced form and IV results
# -------------------------------------------------------------------------------------------------------------------

# Make a directory to hold outputs
dir.create("Plots/FigED8/")
...
# get some errors in here for NAs, NBD
cl = signif(cl,2)
ng = signif(ng,2)
nc = signif(nc,2)
cse = signif(cse,2)
...
ind = c(3,4,7,10,13,16)
ord2 = ord[ind]
for (i in 1:length(ord2)) {
  png(width=3,height=3.5,units="in",res=300,file=paste("Plots/FigED8/ReducedForm_",ord2[i],".png",sep=""))
    par(mar=c(5,4,3,1),las=1)
    rg = max(abs(min(cl[ii,ind[i]],ng[ii,ind[i]],na.rm=T)),max(cl[ii,ind[i]],ng[ii,ind[i]],na.rm=T))
    ...
    segments(x0=cl[ii,ind[i]]-sig*cse[ii,ind[i]],x1=cl[ii,ind[i]]+sig*cse[ii,ind[i]],y0=yy,col="black",lty=1)
    points(cl[ii,ind[i]],y=yy,pch=pp,col="black",bg=col,cex=1.5)
    segments(x0=ng[ii,ind[i]]-sig*ngse[ii,ind[i]],x1=ng[ii,ind[i]]+sig*ngse[ii,ind[i]],y0=(yy)-0.1,col="black",lty=1)
    points(ng[ii,ind[i]],y=(yy)-0.1,pch=pp,col="black",bg="blue",cex=1.5)
    segments(x0=nc[ii,ind[i]]-sig*ncse[ii,ind[i]],x1=nc[ii,ind[i]]+sig*ncse[ii,ind[i]],y0=(yy)+0.1,col="black",lty=1)
    points(nc[ii,ind[i]],y=(yy)+0.1,pch=pp,col="red",bg="white",cex=1.5)
    
    segments(x0=mncl[ind[i]]-sig*mncse[ind[i]],x1=mncl[ind[i]]+sig*mncse[ind[i]],y0=0.6,col="darkgrey",lwd=2)
    segments(x0=mncl[ind[i]]-sig*mncse[ind[i]],y0=0.55,y1=0.65,col="darkgrey",lwd=2)
    segments(x0=mncl[ind[i]]+sig*mncse[ind[i]],y0=0.55,y1=0.65,col="darkgrey",lwd=2)
    points(mncl[ind[i]],0.6,col="darkgrey",lwd=2,pch=16,cex=1)
    box(lwd=2)
  dev.off()
}

# Do pm and pmsurf IVs together
png(width=3,height=3.5,units="in",res=300,file="Plots/FigED8/PMFirstStage.png")
  par(mar=c(5,4,3,1),las=1)
...
for (i in 1:length(varlist[outcomes])) {
  ...
  png(width=3,height=3.5,units="in",res=300,file=paste("Plots/FigED8/IV_",varlist[outcomes[i]],".png",sep=""))
    par(mar=c(5,4,3,1),las=1)
